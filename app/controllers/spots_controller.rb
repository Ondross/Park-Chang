...
i = 0
@spots.each do |spot|
  marker = Cartographer::Gmarker.new(:name=> "taj#{i}", :marker_type => "Building",
              :position => [spot.lat,spot.long],
              :info_window_url => "/spots/#{spot.id}",
              :icon => icon_building)
i += 1
...
#    @map.markers << marker2
  ...
  def upvote
if (pq = Spot.find_by_id(params[:id]))
  pq.rating = pq.rating + 1
  pq.save!
end
	@spot = Spot.find(params[:id])

	    respond_to do |format|
        format.html { redirect_to(@spot, :notice => 'Thanks for your input.') }
...
if (pq = Spot.find_by_id(params[:id]))
  ...
  pq.save!
	...
	@spot = Spot.find(params[:id])
	    ...
	    respond_to do |format|
        format.html { redirect_to(@spot, :notice => 'I thought something was up with that.') }
      format.xml  { render :xml => @spot }
    end
  end
